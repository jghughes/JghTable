...
<#
.SYNOPSIS
    Builds a single TypeScript app and uploads its static output to Azure Blob Storage.

.DESCRIPTION
    Follows the same pattern as the HP21 deploy script: validate the environment,
    do a clean build, preview deletions, require explicit confirmation, upload only the
    selected artifacts, and print the public URL at the end.
...
$sourceFolder = Join-Path $projectDir $App
$containerName = '$web'
...
Write-Host "Cleaning previous build output for '$App'..." -ForegroundColor Cyan
if (Test-Path $distPath) {
    Remove-Item -Path $distPath -Recurse -Force
    Write-Host "  Removed: $distPath" -ForegroundColor Gray
}
else {
    Write-Host "  No dist folder found for '$App'" -ForegroundColor Gray
}

Write-Host "Ensuring shared framework is rebuilt before app compilation..." -ForegroundColor Cyan
& npx tsc --build (Join-Path $projectDir "shared-mastercopies\tsconfig.json") --force
if ($LASTEXITCODE -ne 0) {
    throw "npx tsc --build for shared-mastercopies failed with exit code $LASTEXITCODE"
}

Write-Host "Building TypeScript project for '$App'..." -ForegroundColor Cyan
...
if (-not (Test-Path $distPath)) {
    throw "Expected compiled dist output at '$distPath' after the build, but it was not created."
}
...
Write-Host ""
Write-Host "The $App site is now live at:" -ForegroundColor Green
